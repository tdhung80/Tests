...
namespace FastAccessProperty
{
    ...
    {
        private static readonly ConcurrentDictionary<ConstructorInfo, Func<object[], object>> ConstructorToWrapperMap = new ConcurrentDictionary<ConstructorInfo, Func<object[], object>>();

        private static readonly ConcurrentDictionary<Type, LambdaInvoker> TypeToWrapperMap = new ConcurrentDictionary<Type, LambdaInvoker>();

        private static readonly ConcurrentDictionary<MethodKey, LambdaInvoker> MethodToWrapperMap = new ConcurrentDictionary<MethodKey, LambdaInvoker>(MethodKeyComparer.Instance);

        private readonly Func<object, object[], object> _func;

        private LambdaInvoker (Func<object, object[], object> func) {
            _func = func;
        }
        ...
        public static Func<object[], object> ForConstructor (ConstructorInfo constructor) {
            ...
            return ConstructorToWrapperMap.GetOrAdd(constructor, t => {
                ParameterExpression argsExp = null;
                Expression[] paramsExps = null;
                CreateParamsExpressions(constructor, out argsExp, out paramsExps);
        ...
        public static LambdaInvoker ForDelegate (Delegate del) {
            if (del == null)
            ...
            return TypeToWrapperMap.GetOrAdd(type, t => {
                ...
                return new LambdaInvoker(wrapper);
            });
        }

        public static LambdaInvoker ForMethod (Type type, string methodName) {
            if (type == null)
        ...
        public static LambdaInvoker ForProperty (Type type, string propertyName) {
            if (type == null)
                throw new ArgumentNullException(nameof(type));
        ...
        private static Func<object, object[], object> CreateMethodWrapper (Type type, MethodInfo method, bool isDelegate) {
            ParameterExpression argsExp = null;
            Expression[] paramsExps = null;
            CreateParamsExpressions(method, out argsExp, out paramsExps);
        ...
        private static void CreateParamsExpressions (MethodBase method, out ParameterExpression argsExp, out Expression[] paramsExps) {
            var parameters = method.GetParameterTypes().ToList();
